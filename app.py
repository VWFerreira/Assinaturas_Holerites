...
def baixar_pdf(file_id):
    try:
        ...
        st.error(f"Erro ao baixar o PDF: {str(e)}")
        return None
        ...
        if link and "drive.google.com" in link and "/" in link:
            ...
            if len(partes) >= 6:
                ...
                if st.session_state.pdf_file is None:
                    st.error("❌ Falha ao carregar o PDF. Verifique se o link está correto.")
                    st.session_state.autenticado = False
            else:
...
with st.container():
    if not st.session_state.autenticado:
        ...
        if canvas_result.image_data is not None and st.button('🖊️ Assinar e Enviar PDF'):
            if st.session_state.pdf_file is None:
                st.error("❌ O PDF não foi carregado corretamente. Verifique se o link está preenchido na planilha.")
                st.stop()
